...
interface NivelacijaTotals {
  ukupno_staro: number;
  ukupno_novo: number;
  ukupno: number;
}

interface PrintModalNivelacijeProps {
  open: boolean;
  onClose: () => void;
  rows: NivelacijaRow[];
  totals: NivelacijaTotals;
  periodLabel: string;
  title: string;
}
  ...
  const rowsHtml = rows
    .map((row, index) => {
      ...
      return `<tr style="background:${bg}">
        <td>${row.sifra_nivelacije ?? "-"}</td>
        <td>${displayDate(row.datum_nivelacije)}</td>
        <td class="num">${fmt(Number(row.ukupno_staro))}</td>
        <td class="num">${fmt(Number(row.ukupno_novo))}</td>
...
<body>
  ...
  <div class="summary">
    <div class="summary-box"><div class="label">Ukupno staro</div><div class="value">${fmt(totals.ukupno_staro)} KM</div></div>
    <div class="summary-box"><div class="label">Ukupno novo</div><div class="value">${fmt(totals.ukupno_novo)} KM</div></div>
    <div class="summary-box"><div class="label">Razlika</div><div class="value">${fmt(totals.ukupno)} KM</div></div>
  </div>

  <table>
    <thead>
      <tr>
        <th>Sifra nivelacije</th>
        <th>Datum nivelacije</th>
        <th class="num">Ukupno staro</th>
        <th class="num">Ukupno novo</th>
    ...
    <tfoot>
      <tr>
        <td colspan="2">UKUPNO - ${rows.length} stavki</td>
        <td class="num">${fmt(totals.ukupno_staro)}</td>
        <td class="num">${fmt(totals.ukupno_novo)}</td>
...
export function PrintModalNivelacije({
  open,
  onClose,
  rows,
  totals,
